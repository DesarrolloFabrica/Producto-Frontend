...
export function createMockSubjects(count = 18): OperationalSubjectV2[] {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    if (['PENDING_FACTORY', 'IN_FACTORY_PRODUCTION', 'PENDING_PLANNING_PRODUCTION_VALIDATION', 'RETURNED_TO_FACTORY_FROM_PLANNING', 'PENDING_LMS_UPLOAD', 'IN_LMS_UPLOAD', 'PENDING_PLANNING_LMS_VALIDATION', 'RETURNED_TO_LMS_FROM_PLANNING', 'PENDING_PRODUCT_ACADEMIC_REVIEW', 'IN_PRODUCT_ACADEMIC_REVIEW', 'CHANGES_REQUESTED_BY_PRODUCT', 'PENDING_FINAL_RADICATION', 'FINALIZED'].includes(operationalState)) {
      mark('PLANNING_INITIAL_VALIDATED', 'PLANEACION', 'Validacion inicial OK.');
    }
    if (['PENDING_PLANNING_PRODUCTION_VALIDATION', 'RETURNED_TO_FACTORY_FROM_PLANNING', 'PENDING_LMS_UPLOAD', 'IN_LMS_UPLOAD', 'PENDING_PLANNING_LMS_VALIDATION', 'RETURNED_TO_LMS_FROM_PLANNING', 'PENDING_PRODUCT_ACADEMIC_REVIEW', 'IN_PRODUCT_ACADEMIC_REVIEW', 'CHANGES_REQUESTED_BY_PRODUCT', 'PENDING_FINAL_RADICATION', 'FINALIZED'].includes(operationalState)) {
      mark('FACTORY_CONTENT_DELIVERED', 'FABRICA', 'Contenido entregado en Drive.');
    }
    if (['PENDING_LMS_UPLOAD', 'IN_LMS_UPLOAD', 'PENDING_PLANNING_LMS_VALIDATION', 'RETURNED_TO_LMS_FROM_PLANNING', 'PENDING_PRODUCT_ACADEMIC_REVIEW', 'IN_PRODUCT_ACADEMIC_REVIEW', 'CHANGES_REQUESTED_BY_PRODUCT', 'PENDING_FINAL_RADICATION', 'FINALIZED'].includes(operationalState)) {
      mark('PLANNING_PRODUCTION_VALIDATED', 'PLANEACION', 'Produccion validada.');
    }
    if (['PENDING_PLANNING_LMS_VALIDATION', 'RETURNED_TO_LMS_FROM_PLANNING', 'PENDING_PRODUCT_ACADEMIC_REVIEW', 'IN_PRODUCT_ACADEMIC_REVIEW', 'CHANGES_REQUESTED_BY_PRODUCT', 'PENDING_FINAL_RADICATION', 'FINALIZED'].includes(operationalState)) {
      mark('LMS_UPLOAD_COMPLETED', 'LMS', 'Carga y publicacion completada.');
    }
    if (['PENDING_PRODUCT_ACADEMIC_REVIEW', 'IN_PRODUCT_ACADEMIC_REVIEW', 'CHANGES_REQUESTED_BY_PRODUCT', 'PENDING_FINAL_RADICATION', 'FINALIZED'].includes(operationalState)) {
      mark('PLANNING_LMS_VALIDATED', 'PLANEACION', 'Validacion LMS OK.');
    }
    if (['PENDING_FINAL_RADICATION', 'FINALIZED'].includes(operationalState)) {
    ...
    const responsibleRole: OperationalRoleV2 =
      operationalState === 'PENDING_PLANNING_INITIAL_VALIDATION' || operationalState === 'PENDING_PLANNING_PRODUCTION_VALIDATION' || operationalState === 'PENDING_PLANNING_LMS_VALIDATION' || operationalState === 'PENDING_FINAL_RADICATION'
        ? 'PLANEACION'
