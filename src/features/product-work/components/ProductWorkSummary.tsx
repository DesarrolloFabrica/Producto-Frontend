...
import { getActiveFilterChips, getSortLabel, getStatusLabel } from '../productWorkConstants';
import type { ProductWorkQuery } from './ProductWorkFilters';

export function ProductWorkSummary({
  total,
  query,
  onRemoveFilter,
  onClearAll,
  ...
  const chips = getActiveFilterChips(query as any);
  const statusLabel = getStatusLabel(query.status);
  const sortLabel = getSortLabel(query.sort);

  const countLabel = total === 1 ? '1 materia encontrada' : `${total} materias encontradas`;
