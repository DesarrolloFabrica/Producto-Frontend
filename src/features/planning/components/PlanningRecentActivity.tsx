...
import type { PlanningActivityItem } from '../../../services/planningApi';

type PlanningRecentActivityProps = {
  items: PlanningActivityItem[];
  /** Ruta al historial completo (p. ej. `/planning/dashboard?filter=history`). */
  historyTo?: string;
  limit?: number;
};

export function PlanningRecentActivity({
  items,
  historyTo = '/planning/dashboard?filter=history',
  limit = RECENT_ACTIVITY_PREVIEW_LIMIT,
}: PlanningRecentActivityProps) {
    ...
    <Card className="overflow-hidden p-0">
      <div className="border-b border-slate-200/60 px-5 py-4 sm:px-6">
        ...
        <p className="mt-1 text-xs font-medium text-slate-500">
          Últimas validaciones, devoluciones y cierres registrados por Planeación.
        </p>
      </div>
      {items.length === 0 ? (
        <p className="px-5 py-8 text-center text-sm text-slate-500 sm:px-6">
          Aún no hay actividad registrada en el flujo institucional.
        </p>
      ) : (
        <>
