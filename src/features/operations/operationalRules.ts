...
import { checklistStatusLabels, projectStatusLabels, subjectStatusLabels } from '../../utils/status';
import type {
...
export function getSubjectBlockers(
  ...
  if (subject.status !== 'APPROVED' && remainingDays <= 3) {
    blockers.push({
...
export function getSubjectNextAction(
  ...
  return {
    title: subject.status === 'APPROVED' ? 'Materia aprobada' : 'Validar si esta lista para aprobacion',
    description: subject.status === 'APPROVED' ? 'No hay acciones criticas pendientes.' : 'El checklist no tiene pendientes criticos visibles.',
    actionLabel: subject.status === 'APPROVED' ? 'Ver trazabilidad' : 'Revisar materia',
    responsibleRole: 'FABRICA',
    impact: subject.status === 'APPROVED' ? 'Aporta al avance de entrega del proyecto.' : 'Confirma si puede avanzar a revision o aprobacion.',
    severity: subject.status === 'APPROVED' ? 'completed' : 'info',
    targetRoute: `/subjects/${subject.id}`,
    targetContext: { type: 'subject', id: subject.id },
  };
}
...
export function getChecklistItemInsight(item: ChecklistItem, project: VirtualizationProject, subject: SubjectVirtualization): OperationalInsight {
  ...
  const byStatus: Record<ChecklistStatus, Omit<OperationalInsight, keyof typeof base>> = {
    ...
    RECHAZADO: {
      ...
      description: `${item.label} fue rechazado y requiere correccion.`,
      severity: 'blocking',
      nextAction: 'Corregir y reentregar el entregable.',
      impact: 'Bloquea aprobacion hasta que se resuelva.',
      dependency: 'Correccion por parte de Fabrica.',
    },
  };
...
export function getSubjectOperationalInsight(
  ...
  return {
    ...
    entityType: 'subject',
    entityId: subject.id,
    title: `Materia: ${subjectStatusLabels[subject.status]}`,
