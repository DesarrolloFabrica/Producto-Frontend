...
import { formatSemesterSubjectBlocker, semesterSubjectInternalStateMeta } from '../institutionalCopy';
import { subjectChecklistPath, subjectFactoryCorrectionsPath } from '../institutionalNavigation';
      ...
      <table className="min-w-full text-sm">
        ...
        <tbody className="divide-y divide-slate-100">
          {subjects.map((subject) => {
            ...
            const hasBlockers = subject.blockers.length > 0;
            ...
            const actionLabel = checklistReviewMode
              ? 'Revisar checklist'
                  ...
                  <div className="space-y-2">
                    ...
                    {!showRequirements && hasBlockers && !hasObservations ? (
                      <ul className="space-y-1">
                        {subject.blockers.slice(0, 2).map((blocker) => (
                  ...
                  <td className="px-5 py-4 align-top">
                    {hasBlockers ? (
                      <ul className="space-y-1.5">
                        {subject.blockers.slice(0, 3).map((blocker) => (
                          <li
                            key={blocker}
