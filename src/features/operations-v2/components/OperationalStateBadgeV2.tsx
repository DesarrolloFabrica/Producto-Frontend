...
export function OperationalStateBadgeV2({ state }: { state: OperationalStateV2 }) {
  ...
  return (
    <span className={cn('inline-flex rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ring-1', tone.bg, tone.text, tone.ring)} title={stateLabelV2(state)}>
      {stateLabelV2(state)}
    </span>
  );
}
