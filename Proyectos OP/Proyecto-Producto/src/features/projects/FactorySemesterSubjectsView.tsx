...
export function FactorySemesterSubjectsView() {
  ...
  const openProductObs = projectObservations.filter(
    (o) => o.authorRole === 'PRODUCT' && (o.status === 'ABIERTA' || o.status === 'OPEN') && subjects.some((s) => s.id === o.subjectId)
  );
                ...
                <div key={obs.id} className="rounded-[16px] bg-white p-4 shadow-[0_2px_12px_-3px_rgba(0,0,0,0.06)] border border-rose-100/50">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-xs text-[#64748B] line-clamp-2">{obs.content}</p>
                    </div>
                    {subject && (
                      <Link
      ...
      {subjects.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {subjects.map((subject) => {
            ...
            const subjectApproved = subject.checklist.filter((c) => c.status === 'APROBADO').length;
            const subjectProgress = subjectChecklist > 0 ? Math.round((subjectApproved / subjectChecklist) * 100) : 0;
