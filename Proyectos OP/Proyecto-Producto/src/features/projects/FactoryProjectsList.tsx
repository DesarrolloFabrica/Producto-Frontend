...
export function FactoryProjectsList() {
  ...
  const filtered = factoryProjects.filter((p) => {
    ...
    if (activeFilter === 'corrections') return p.status === 'FEEDBACK_PENDING' || openProductObs.some((o) => o.relatedEntity?.includes(p.id));
  ...
  const counts = {
    ...
    corrections: factoryProjects.filter((p) => p.status === 'FEEDBACK_PENDING' || openProductObs.some((o) => o.relatedEntity?.includes(p.id))).length,
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((project) => {
            const obs = openProductObs.filter((o) => o.relatedEntity?.includes(project.id));
            return (
              <div
                key={project.id}
              ...
              >
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <span className={cn(
                    ...
                    project.priority === 'ALTA' ? 'bg-rose-50 text-rose-600' : project.priority === 'MEDIA' ? 'bg-amber-50 text-amber-600' : 'bg-slate-50 text-slate-500'
                  )}>
