...
type OperationsAction =
  ...
  | { type: 'ADD_TOPIC_TO_SUBJECT'; payload: { projectId: string; subjectId: string; topicName: string; checklist: VirtualizationProject['subjects'][0]['checklist'] } }
...
function operationsReducer(state: OperationsState, action: OperationsAction): OperationsState {
  switch (action.type) {
    ...
    case 'ADD_OBSERVATION':
      return {
        ...state,
        projectObservations: [action.payload.observation, ...state.projectObservations],
        auditLogs: [
          {
            id: `aud-${Date.now()}`,
        ...
        projects: state.projects.map((project) => {
          ...
          return {
            ...
            subjects: project.subjects.map((subject) => {
              ...
              return {
                ...subject,
                checklist: subject.checklist.map((item) => (item.id === action.payload.checklistItemId ? { ...item, status: action.payload.newStatus, updatedAt: new Date().toISOString() } : item)),
              };
            }),
          };
        }),
    ...
    case 'ADD_SUBJECT_TO_SEMESTER':
      return {
        ...
        projects: state.projects.map((p) =>
          p.id === action.payload.projectId
            ? { ...p, subjects: [...p.subjects, action.payload.subject] }
            : p,
        ),
        auditLogs: [
          {
    ...
    case 'RESOLVE_OBSERVATION':
      return {
        ...state,
        projectObservations: state.projectObservations.map((o) =>
          o.id === action.payload.observationId
            ? { ...o, status: 'RESUELTA' as const }
            : o,
        ),
        auditLogs: [
          {
...
interface OperationsContextValue extends OperationsState {
  ...
  addTopicToSubject: (projectId: string, subjectId: string, topicName: string, checklist: VirtualizationProject['subjects'][0]['checklist']) => void;
  ...
  updateFactoryTopicChecklistItem: (projectId: string, subjectId: string, topicIndex: number, itemLabel: string, newStatus: ChecklistStatus) => void;
...
export function OperationsProvider({ children }: { children: ReactNode }) {
  ...
  const addTopicToSubject = useCallback((projectId: string, subjectId: string, topicName: string, checklist: VirtualizationProject['subjects'][0]['checklist']) => {
    dispatch({ type: 'ADD_TOPIC_TO_SUBJECT', payload: { projectId, subjectId, topicName, checklist } });
  }, []);
  ...
  const updateFactoryTopicChecklistItem = useCallback((projectId: string, subjectId: string, topicIndex: number, itemLabel: string, newStatus: ChecklistStatus) => {
    const project = state.projects.find((p) => p.id === projectId);
    if (!project) return;
    const subject = project.subjects.find((s) => s.id === subjectId);
    if (!subject) return;
    const topicName = subject.contentTopics?.[topicIndex];
    if (!topicName) return;
    dispatch({
      type: 'ADD_ACTIVITY_EVENT',
      payload: {
        ...
        userName: 'Fábrica',
        role: 'FABRICA',
        action: `actualizó checklist del tema "${topicName}"`,
        entityType: 'Checklist tema',
        entityName: `${subject.name} - ${itemLabel}`,
        eventType: 'STATUS',
        projectId,
        subjectId,
        createdAt: new Date().toISOString(),
  ...
  const markObservationCorrectionApplied = useCallback((projectId: string, observationId: string, observation: OperationalObservation) => {
    ...
    dispatch({
      ...
      payload: {
        id: `not-${Date.now()}`,
        title: 'Fábrica aplicó corrección',
        message: `Fábrica aplicó corrección en ${observation.relatedEntity}.`,
        roleTarget: 'PRODUCT',
        type: 'INFO',
        createdAt: new Date().toISOString(),
        read: false,
  ...
  const markSubjectDelivered = useCallback((projectId: string, subjectId: string, subjectName: string) => {
    ...
    dispatch({
      ...
      payload: { id: projectId, newStatus: 'IN_REVIEW', oldStatus: project.status, project },
    });
    dispatch({
      type: 'ADD_NOTIFICATION',
      payload: {
